...
export default function WeatherScreen() {
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 bg-[#4f7cff]" edges={['top', 'bottom']}>
    ...
    >
      <View className="flex-1 px-6">
        <View className="mb-8 flex-row items-center justify-between">
          ...
          <Pressable onPress={getWeather} className="h-10 w-10 items-end justify-center">
        ...
        <View className="items-center rounded-[32px] border border-white/35 bg-white/20 px-6 py-[30px]">
          ...
          <Text className="mt-4 text-[70px] font-thin text-white">
            {temp}°
          </Text>
        ...
        <View className="mt-8 flex-row flex-wrap justify-between">
          <InfoCard icon="thermometer" label="체감온도" value={`${feelsLike}°C`} />
          <InfoCard icon="water" label="습도" value={`${weather.main.humidity}%`} />
          <InfoCard icon="speedometer" label="풍속" value={`${weather.wind.speed} m/s`} />
          <InfoCard icon="cloud" label="기압" value={`${weather.main.pressure} hPa`} />
        </View>
      </View>
    </SafeAreaView>
  );
